...
>
> **Gates: 677 tests passing (was 655), build clean both flag-on and flag-off, lint 140 (baseline 141 — no new).** Headless-verified: dark stage + lamp pool render, `--ink` is `#E8E4DD` in Act I, zero new runtime errors.
>
> **Outstanding.** **(1) Generate the TTS** — the 14 Prompter clips are authored + wired but **not on disk**; `node scripts/generate-admirer-voice.js` with `ELEVENLABS_API_KEY` in `.env.local` renders them. Until then every line is fail-silent and Act I behaves as it does today. **(2) `season-door` is authored but unwired** — the Coda's Season/journal door does not exist in `Settle.jsx` (only the constellation door does); the line waits for that door to be built. **(3) `bloom-hall` is authored but unwired** — a voice line at the sacred seam would be cut off by the ~600ms phase swap; wiring it needs a deliberate design decision, not a drive-by. **(4) The on-device pass** (`docs/prod-test-checklist.md`) — whether the spoken asks land at the right moment, whether the fade leaves the gesture discoverable, and whether ducking keeps the voice intelligible over each bed.
